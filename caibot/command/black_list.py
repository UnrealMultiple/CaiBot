...
from caibot.utils import ban_request_to_str
...
async def _(bot: Bot, args: Args, session: Session, group: Group, user: User,
    ...
    request_msg = await ban_request_to_str(ban_request)
    # noinspection PyUnresolvedReferences
    report_msg = MessageSegment.text(f"🙏云黑请求{request_msg}")

    await bot.send_group_message(group_id=group.group_id, message=report_msg)
